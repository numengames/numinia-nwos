...
repo-local, scratch — deleted at mission close per `PRO-010`). Execution
...
tracked in `missions/MIS-0127-entropy-reduction.md` (ledger, per PR) as each
...
2. **`registration: exempt` scope.** 50 files carry the exemption; 24 are
   ...
   in error (it is apparatus of `PRO-008`, not orphaned) — caught and
...
- **`count-evidence.py` and `PRO-010` disagreed about what a frozen artefact
...
| `missions/MIS-0118-agent-roster-replacement.md` | `status: done` mission | retrospective narrative: it records what a guard run found on 2026-08-28, under the names of that date |
| `missions/MIS-0125-prefix-register.md` | rewrote `charter-alchemists`, `roster-sentinels` → `GLD-001`, `GLD-006` | those ids appear in §"What this mission decides" **as examples of lowercase descriptive ids**. The sentence became "whether lowercase descriptive ids are legal at all — `GLD-001`, `GLD-006`" — self-refuting |
...
permanent policy in **`PRO-010` §3.4 — a citation may be rewritten, a mention may
...
`PRO-013` handoff is open: steps 1, 2 and 4 are done (the suite is tested in both
