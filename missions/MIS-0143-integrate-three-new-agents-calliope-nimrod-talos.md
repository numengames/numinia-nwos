...
1. **Calliope** — copywriter, specialist in copywriting and editorial content. Exegetes guild, Chroniclers branch.
2. **Nimrod** (new, unrelated to the retired Centinela-01/Nimrod) — guide and caretaker of all archive files. Sentinels guild, Seraphim branch.
3. **Talos** — security specialist for repo protocols and processes (CI/CD, safety, care systems). Sentinels guild, Archangels branch.
...
`SOUL.md` for each agent starts as a placeholder based on the template, ready to be replaced with the full identity document when the Oracle provides it. All other documents are fully specified.
...
✓  agents/nimrod/AGENT.yaml exists with status: designed and role: archive-guardian
✓  agents/talos/AGENT.yaml exists with status: designed and role: security-specialist
...
✗  agents/calliope/SOUL.md contains full identity prose (placeholder only — Oracle supplies)
```
