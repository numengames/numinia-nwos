...
3. Today's criterion prevails over `Read_Me_How_to_Archive` v0.1.12/v0.2.0;
   what was worth keeping is already distilled into **PRO-010 (draft)**.
...
| D3 | `blueprints/archive-summa-{fundacional,arquitectura,prompt}-v0.1.0.md` | They are archive fondo, not blueprints: move to a fondo (PRO-010 §3.2 nomenclature); exact destination in F1 |
...
| D5 | Three naming conventions coexisting (IDs, dated, free names with spaces: `CAN-001-welcome-to-numinia.md`, `CAN-002-brand-and-culture.md`…) | Batch rename per PRO-010 §3, with a mapping record and redirects where URLs change. **canon/ closed by ADR-036 (2026-09-01)**: the whole folder is `CAN-NNN` now |
...
| D7 | `protocols/2026_04_14-Read_Me_How_to_Archive-v0.2.0.md` superseded by PRO-010 | When PRO-010 is signed: mark superseded and move to a fondo per the §5 lifecycle |
| D8 | Files without frontmatter (~32 per the guard: README, INDEX, templates…) | Give them minimal frontmatter (PRO-010 §3.3) or declare them template/fondo explicitly |

## Phases (each signed separately)

- **F0 — The protocol.** Review and sign PRO-010 v0.3.0 (draft already
...
- [ ] PRO-010 signed (F0) and v0.2.0 marked superseded (D7).
...
- **Evidence:** Master IA plan of 2026-08-18. Of its register D1–D8: D1 resolved 2026-08-25; D2 (AUDIT blueprints → reports) done by MIS-071/ADR-005 v1.2.0; D3 (archive-summa-* → fondo) done by ADR-035 (history/); D4 (/reportes from reports/) done 2026-09-01 (#195); D5 (naming) done by MIS-125/127; D6 (INDEX.md retirement) done #177/#194; D7 (2026_04_14 Read_Me → superseded) done — the file is gone from protocols/; D8 (frontmatter) done by MIS-121/127. PRO-010 is still 'draft' v0.8.2 — its F0 gate ('P-010 signed') never happened formally. 16 citations (8 files).
- **Recommendation:** Close as done — achieved through MIS-109/121/125/127/129 and ADR-005/035, every register line executed; the one open item (PRO-010 status: draft → active, a signature) goes to MIS-135. The file is the plan the last three weeks followed; fill Execution log with the mission ids that did each line.
