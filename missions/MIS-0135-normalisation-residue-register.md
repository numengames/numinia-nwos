...
| 5 | B | `standards/STD-004-header-standard.md` §7 ring table | `assigned_to` is defined as *"agent-id, or null"*; the corpus carried a repository name (`numinia-nwos` ×3, `nwos-workspace-template`, `numengames-web + nwos-deploy`, `numinia-web`) and a model name (`claude-fable-5` ×2) as assignees. Fixed on the live missions in PR #198 (→ null); the `done` ones keep theirs as record. No guard checks the value. | `grep -h '^assigned_to' missions/MIS-*.md \| sort \| uniq -c` | **defer** → new guard rule (H-39 `assigned_to` ∈ roster) needs STD-004 §7.2 text + roster source; one small mission, Sentinels. Interim: #198 nulled the 8 retired assignees; live values today: null ×30, ursa ×2. |
