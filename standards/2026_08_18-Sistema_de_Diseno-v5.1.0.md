...
| **Engranajes de valoración** [5.1.0 · H2] | fila de engranajes Phosphor `gear` 0–5, `regular` 18 px; los alcanzados en `fill`; al fijar el valor, los fijados giran **un cuarto de vuelta** (120 ms, elevación 04) | Valoración de ficha (MIS-085); solo donde el usuario valora — nunca para mostrar datos que no fijó él; con `prefers-reduced-motion`, sin giro |
| **El Narrador** [5.1.0 · H3] | play/pausa en la barra del libro (icono propio del set §7.5); lee en voz alta vía Web Speech, **resalta el bloque leído** (filo solar, como el término de glosario alcanzado), control de ritmo junto al A·A·A | Alcance: cuerpo del códex, glosario y ficha — nunca interfaz; si Web Speech no está disponible, el control no se muestra (nada de botones muertos); el resalte sigue al audio, no al scroll |
...
| **12** | **Paso de página** — **RETIRADA en 5.1.0 (H5)** | La 5.0.0 la registró «por verificar contra el LAP»; la verificación dio vacío: el códex no anima el paso de hoja — lo único vivo en esa vista es el Trazo (13). El número no se reutiliza (catálogo append-only); si algún día el papel pasa hoja con animación, entrará como pieza nueva con specs propias | — | — |
...
- **Las tintas del libro** [5.1.0 · H1]: el texto terciario del sistema (`#6E6259`/`#8A7D72`) se queda en 3.7:1 sobre el papel del códex — bajo AA. Dentro del registro papel, el terciario es `#75695E` (Diurno) y `#97897D` (Nocturno) — token `papel.tinta-terciaria` (§19.3), verificado en producción. Fuera del papel, las tintas del sistema siguen intactas.
- **Las ediciones son plano, no cortesía** [5.1.0 · H4]: el **pdf** se produce por impresión del CSS del sitio (`@media print`, A4 — el códex completo imprime como un libro real, ~413 páginas); la **ficha imprimible** sale sin barra de acciones ni controles (el papel no lleva botones); el **epub** conserva los enlaces de glosario. Ninguna edición se genera con maquetador aparte: la fuente es una, el CSS es el mismo.
- **Las piezas editoriales** (capitular, cuadro de lectura, lámina, tabla numerada, nota al margen, filete, sello, glosario, descargas, colofón, engranajes de valoración, Narrador) están especificadas en §9.9; los iconos del libro, en §7.5; el grano, en §6.5.
...
|---|---|---|
| **5.1.0** | 2026-08-18 | **El emisor gobierna: erratas, huecos de producción y el kit regenerado.** MINOR de consolidación sobre la 5.0.0, por encargo del consumidor (numinia-web, ADR-022) tras fijar este repo como fuente. **Erratas (E1–E5):** el fragmento §19.5 pasa de «doce» a **trece** animaciones nombrando trazo y cielo; radio marco corregido 10→**8 px** (fósil de la 4.0.0); §6.4 recolocada antes que §6.5; presupuesto de lectura medido (documento ≈46k tokens · §19 ≈7,5k · §19.5 ≈2,1k); guía viva regenerada con `kit/sistema.*` (la tabla §0.4 conserva `khepri.*` solo en la columna «Antes»). **Huecos de producción (H1–H5):** §13.12/§19.3 ganan **las tintas del libro** (`papel.tinta-terciaria` `#75695E`/`#97897D` — el terciario del sistema daba 3.7:1 sobre papel, bajo AA; el borrador 4.3.0 las llevaba y la 5.0.0 las perdió); §9.9 gana **los engranajes de valoración** (0–5, cuarto de vuelta al fijar, MIS-085) y **el Narrador** (Web Speech, resalte del bloque leído, ritmo, play/pausa en barra, alcance a glosario y ficha); §13.12 declara **las ediciones como plano** (pdf por `@media print` A4 del propio CSS, ficha imprimible sin barra, epub con enlaces de glosario); la animación **12 se retira** — la verificación contra el LAP dio vacío, el número no se reutiliza. **Kit:** `sistema.{css,js,tokens.json}` regenerados del documento vigente (grupos `velo`, `papel`, `registros` dentro), publicados bajo ruta versionada con `manifest.json` (sha256 por fichero); Alegreya **entera** autoalojada (redonda, itálica, SC 400/500) con su OFL. Primer caso de la doctrina de emisión (MIS-068): el canon no se copia — se fija. |
