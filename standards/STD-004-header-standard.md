...
- v2.0.0 (2026-09-03) — the standard keeps every field rule and every check
  identifier, and loses the argument around them. Removed: the opening census
  of a tree measured a week ago, with counts that were already wrong and that
  a normative document should not carry; the amendment note recording two
  transcription errors caught during implementation; the migration table of
  drifting type values, which is a work list and belongs to its migration; the
  narrative of vocabulary drift; and the scope note about who may edit the
  pipeline, which the guard-handoff protocol governs. Section 10's rule on
  cross-references now defers to the plain-writing standard by name instead of
  restating it.
- v1.6.0 and earlier — see git history.
