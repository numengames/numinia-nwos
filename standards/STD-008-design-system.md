...
> is cited as binding, so it is a standard and now says so — `STD-008`, with
> the lifecycle every normative document has. Its `status: draft` records what
> the header already said in Spanish: consolidated on top of v5.0.0, pending
> Oracle signature. No rule of the system changed in this move; only its
> registration did.
>
> The header fields that described the system itself — palette registers,
> typography, iconography, token format, reading budget — are not governance
> metadata and were never registered for `standards/`. They are content, and
> content belongs in the body. What follows is that content, unchanged.

## 0. What this system is

| | |
|---|---|
| Derives from | Numen Design System v4.2.0 + Numinia Brand and Culture v0.1.2 |
| Registers | 1 Umbral · 2 Velo · 3 Low-poly · 4 Píxel |
| Direction | Solarpunk 40 · Steampunk 40 · Cyberpunk 20 |
| Pixel register | Píxel-16 + Pixelify Sans + 90s production guide (graphic-adventure heritage) |
| Typography | Geist + Geist Mono (vercel.com/font, SIL OFL 1.1) · third voice Alegreya book-only [EXTENSION — validate] |
| Iconography | Phosphor Icons (phosphoricons.com, MIT) |
| Token format | W3C Design Tokens (DTCG) + CSS Custom Properties |
| Normative conventions | RFC 2119 (MUST / SHOULD / MAY) |
| Canonical language | English (`ADR-023`) |
| Licence | CC0 1.0 Universal (Legal by Design) · trademarks excluded |
| Review | every six months, alongside Brand & Culture |

**Version 5.0.0 is retired from the repository in this same decision.** This
document becomes the only version of the Design System in `numinia-nwos`.
One consumer is still pending re-anchor: `numinia-web/design-source.json` is
pinned to v5.0.0 and its guard fails with HTTP 404 until re-pinned to this
document.

**No codename anymore.** The working name was retired in 5.0.0; the system is called by what it is.
