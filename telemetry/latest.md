...
- head: `ef272f7`  · corpus_hash: `b0e31ec35d3f7e8c…`  · measured_at: 2026-09-04T11:23:58Z  · root_dirty: 0
...
| `series.registered_total` | 235 | documents | sum of `registration[*].registered` |
| `series.registrable_total` | 235 | documents | sum of `registration[*].total` |
...
| key | value | unit | definition |
|---|---|---|---|
| `missions.total` | 143 | missions | files `missions/MIS-NNNN-*.md` with a frontmatter block |
...
| `tokens.total` | 603315 | tokens | Σ tokens over the corpus (every tracked .md outside web/, whole file, frontmatter included) |
| `tokens.by_dir` | (table below) | tokens | tokens per top-level dir, largest first |
| `tokens.by_status` | (table below) | tokens | tokens per frontmatter status ((none) = no status), largest first |
| `tokens.missions_share_pct` | 40.67 | percent | 100·tokens(missions/)/total, rounded to 0.01 |
...
| `legacy.referencias_textuales_total` | 2819 | mentions | occurrences of `(MIS\|ADR\|DEC\|RPT\|AUD\|P\|C\|BP)-<1..4 digits>` at word boundaries in the full text of docs_total (frontmatter included) |
...
|---|---|---|---|---|---|
| missions | MIS-NNNN | 143 | 143 | 1 | 100 |
