...
- head: `af691bf`  · corpus_hash: `ad4f959b86591c11…`  · measured_at: 2026-09-07T14:23:29Z  · root_dirty: 0

## corpus

| key | value | unit | definition |
|---|---|---|---|
| `corpus.files_total` | 599 | files | `git ls-files` at HEAD, every path |
| `corpus.files_by_ext` | (table below) | files | tracked files by lowercase extension; `(none)` when no extension |
| `corpus.md_total` | 290 | files | tracked `.md` anywhere, including `web/` |
| `corpus.docs_total` | 288 | documents | tracked `.md` outside `web/` — the corpus every other family measures |
...
| `series.registered_total` | 221 | documents | sum of `registration[*].registered` |
| `series.registrable_total` | 221 | documents | sum of `registration[*].total` |
...
| `tokens.total` | 538103 | tokens | Σ tokens over the corpus (every tracked .md outside web/, whole file, frontmatter included) |
| `tokens.by_dir` | (table below) | tokens | tokens per top-level dir, largest first |
| `tokens.by_status` | (table below) | tokens | tokens per frontmatter status ((none) = no status), largest first |
| `tokens.missions_share_pct` | 50.45 | percent | 100·tokens(missions/)/total, rounded to 0.01 |
...
| key | value | unit | definition |
|---|---|---|---|
| `headers.docs_with_frontmatter` | 281 | documents | docs whose text opens with a --- block the shared reader parses |
...
| key | value | unit | definition |
|---|---|---|---|
| `figures.live` | 453 | lines | lines in non-apparatus docs outside telemetry/ (frontmatter and code fences excluded) that state a corpus-shaped figure — "N tokens\|documents\|files\|missions", "N/M", "N %" — with no `@ <7-hex head>` on the line. A detector, not a verdict: dated tables and closed records legitimately carry such lines |
...
| missions/MIS-0146-normative-refoundation.md | 19 |
...
| key | value | unit | definition |
|---|---|---|---|
| `legacy.docs_total` | 290 | documents | every tracked path ending in .md (git ls-files '*.md'), web/ included, telemetry/ excluded |
| `legacy.docs_con_frontmatter` | 281 | documents | docs_total whose text starts with a `---` block closed by a second `---` line |
| `legacy.docs_sin_frontmatter` | 9 | documents | docs_total − docs_con_frontmatter |
| `legacy.referencias_textuales_total` | 2171 | mentions | occurrences of `(MIS\|ADR\|DEC\|RPT\|AUD\|P\|C\|BP)-<1..4 digits>` at word boundaries in the full text of docs_total (frontmatter included) |
...
| `legacy.created_total` | 280 | documents | docs with a `created:` line |
...
| standards | STD-NNN | 11 | 11 | 1 | 100 |
| operations | OPS-NNN | 9 | 9 | 0 | 100 |
| debt | DBT-NNN | 8 | 8 | 0 | 100 |
