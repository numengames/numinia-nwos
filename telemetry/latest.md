...
- head: `4577603`  · corpus_hash: `02a4850e3c0d8c52…`  · measured_at: 2026-09-03T09:59:26Z  · root_dirty: 0

## corpus

| key | value | unit | definition |
|---|---|---|---|
| `corpus.files_total` | 584 | files | `git ls-files` at HEAD, every path |
| `corpus.files_by_ext` | (table below) | files | tracked files by lowercase extension; `(none)` when no extension |
| `corpus.md_total` | 278 | files | tracked `.md` anywhere, including `web/` |
| `corpus.docs_total` | 276 | documents | tracked `.md` outside `web/` — the corpus every other family measures |
...
| `corpus.scripts_total` | 43 | files | files under `scripts/` with a code extension (.py .mjs .js .sh .ts) |
...
| `series.registered_total` | 228 | documents | sum of `registration[*].registered` |
| `series.registrable_total` | 228 | documents | sum of `registration[*].total` |
...
| key | value | unit | definition |
|---|---|---|---|
| `missions.total` | 140 | missions | files `missions/MIS-NNNN-*.md` with a frontmatter block |
...
| `tokens.total` | 617346 | tokens | Σ tokens over the corpus (every tracked .md outside web/, whole file, frontmatter included) |
| `tokens.by_dir` | (table below) | tokens | tokens per top-level dir, largest first |
| `tokens.by_status` | (table below) | tokens | tokens per frontmatter status ((none) = no status), largest first |
| `tokens.missions_share_pct` | 39.81 | percent | 100·tokens(missions/)/total, rounded to 0.01 |
...
| in-review | 11131 |
| superseded | 1184 |
| draft          # draft\|active\|closed   (a standard opens at draft) | 1169 |
...
| key | value | unit | definition |
|---|---|---|---|
| `headers.docs_with_frontmatter` | 270 | documents | docs whose text opens with a --- block the shared reader parses |
...
| `contradictions.ci_markers_std001` | 7 | rows | table rows of STD-001 carrying `[CI]` |
...
| in-progress | 9 |
| in-review | 2 |
| draft          # draft\|active\|closed   (a standard opens at draft) | 1 |

### `contradictions.status_vocabulary_undeclared`

| | 0 |
|---|---|
| draft          # draft\|active\|closed   (a standard opens at draft) | templates/STD-TEMPLATE.md |
...
| key | value | unit | definition |
|---|---|---|---|
| `figures.live` | 665 | lines | lines in non-apparatus docs outside telemetry/ (frontmatter and code fences excluded) that state a corpus-shaped figure — "N tokens\|documents\|files\|missions", "N/M", "N %" — with no `@ <7-hex head>` on the line. A detector, not a verdict: dated tables and closed records legitimately carry such lines |
...
| | lines |
|---|---|
| standards/2026_08_18-Sistema_de_Diseno-v5.1.0.md | 41 |
| standards/STD-001-glossary.md | 39 |
...
| key | value | unit | definition |
|---|---|---|---|
| `legacy.docs_total` | 278 | documents | every tracked path ending in .md (git ls-files '*.md'), web/ included, telemetry/ excluded |
| `legacy.docs_con_frontmatter` | 270 | documents | docs_total whose text starts with a `---` block closed by a second `---` line |
| `legacy.docs_sin_frontmatter` | 8 | documents | docs_total − docs_con_frontmatter |
| `legacy.referencias_textuales_total` | 3017 | mentions | occurrences of `(MIS\|ADR\|DEC\|RPT\|AUD\|P\|C\|BP)-<1..4 digits>` at word boundaries in the full text of docs_total (frontmatter included) |
...
| `legacy.uid_presentes` | 39 | documents | docs with a non-empty `uid:` line in the frontmatter |
...
| `legacy.uid_colisiones` | 37 | collisions | Σ(count − 1) over uid values held by more than one doc. Known defect reproduced: values are compared as typed, so a shared placeholder counts as collisions (MIS-122) |
...
| `legacy.created_total` | 268 | documents | docs with a `created:` line |
...
|---|---|---|---|---|---|
| missions | MIS-NNNN | 140 | 140 | 4 | 100 |
