...
- head: `6b0cc03`  · corpus_hash: `6a178ab4b716a4c7…`  · measured_at: 2026-09-04T12:03:02Z  · root_dirty: 0
...
| `corpus.md_total` | 282 | files | tracked `.md` anywhere, including `web/` |
| `corpus.docs_total` | 280 | documents | tracked `.md` outside `web/` — the corpus every other family measures |
...
| `corpus.apparatus` | 12 | documents | corpus documents classified apparatus by rules.json (`type: meta`, listed basename, or template path) |
...
| `series.registered_total` | 236 | documents | sum of `registration[*].registered` |
| `series.registrable_total` | 236 | documents | sum of `registration[*].total` |
...
| key | value | unit | definition |
|---|---|---|---|
| `missions.total` | 144 | missions | files `missions/MIS-NNNN-*.md` with a frontmatter block |
...
| `missions.unassigned` | 94 | missions | `assigned_to` absent or null-like |
...
| `tokens.total` | 604431 | tokens | Σ tokens over the corpus (every tracked .md outside web/, whole file, frontmatter included) |
| `tokens.by_dir` | (table below) | tokens | tokens per top-level dir, largest first |
| `tokens.by_status` | (table below) | tokens | tokens per frontmatter status ((none) = no status), largest first |
| `tokens.missions_share_pct` | 40.78 | percent | 100·tokens(missions/)/total, rounded to 0.01 |
...
| `provenance.dates_vs_commits_compared` | 268 | documents | docs with a created date AND a first-add commit found by one `git log --diff-filter=AR -M` walk (renames followed) |
...
| `provenance.created_behind_commit` | 43 | documents | created day earlier than the first-add commit — expected for migrated or backdated documents; counted, not judged |
...
| key | value | unit | definition |
|---|---|---|---|
| `figures.live` | 623 | lines | lines in non-apparatus docs outside telemetry/ (frontmatter and code fences excluded) that state a corpus-shaped figure — "N tokens\|documents\|files\|missions", "N/M", "N %" — with no `@ <7-hex head>` on the line. A detector, not a verdict: dated tables and closed records legitimately carry such lines |
...
| key | value | unit | definition |
|---|---|---|---|
| `legacy.docs_total` | 282 | documents | every tracked path ending in .md (git ls-files '*.md'), web/ included, telemetry/ excluded |
...
| `legacy.referencias_textuales_total` | 2821 | mentions | occurrences of `(MIS\|ADR\|DEC\|RPT\|AUD\|P\|C\|BP)-<1..4 digits>` at word boundaries in the full text of docs_total (frontmatter included) |
...
|---|---|---|---|---|---|
| missions | MIS-NNNN | 144 | 144 | 1 | 100 |
